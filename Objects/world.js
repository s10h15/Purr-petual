...
import { isRayIntersectsCircle, isRayIntersectsLine, isPointInPolygon, closestSegmentPoint } from "/Helpers/collision.js";
import { getRotatedDirection, getDistance, getPath } from "/math.js";
...
export class World {
	...
	drawBackgroundNoise(ctx, offsetX, offsetY, zoomInRate, width, height) {
		...
		for (let cellY = startY; cellY <= endY; ++cellY) {
			for (let cellX = startX; cellX <= endX; ++cellX) {
				...
				const brightness = 41 + ((seed & 0xFF) % 21) - 10;
	...
	draw(ctx, offsetX, offsetY, zoomInRate, halfWidth, halfHeight, width, height, maxDimension) {
		...
		if (visibilityPolygon.length > 0) {
			setSeed(this.shadowNoiseSeed.toString());
